...
const NAV = [
  ...
  { id:'ocorrencias',  icon:AlertCircle,     label:'Ocorrências',         group:'main',  badgeKey:'ocorrencias' },
  { id:'transacional', icon:Bell,            label:'Gatilhos',            group:'main'   },
  { id:'disparos',     icon:Zap,             label:'Disparos',            group:'main'   },
...
export default function Shell() {
  ...
  useEffect(() => {
    const check = async () => {
      try {
        ...
        if (ro.ok) { const d = await ro.json(); setOcorrCount(d.ocorrencias?.length||0) }
  ...
  const PAGES = {
    ...
    ocorrencias:  <PageOcorrencias  api={API} />,
    transacional: <PageTransacional api={API} />,
    disparos:     <PageDisparos     api={API} />,
        ...
        <div className="p-3" style={{ borderTop:'1px solid var(--sep)' }}>
          ...
          <div className="space-y-2 mb-3">
            {[
              ...
              { n:'Nuvemshop',  ok:online  },
              { n:'Bling ERP',  ok:blingOk, warn:'Reconectar' },
            ].map(s => (
