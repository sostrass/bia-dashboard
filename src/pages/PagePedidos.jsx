...
// Badge de situação — usado em SmartOrderCard (ReferenceError em produção)
function SitBadge({sitId, small}) {
  const s = SIT[String(sitId)] || {label:'—', cor:'var(--label-4)', bg:'transparent'}
  return <span style={{fontSize:small?9:10,fontWeight:700,color:s.cor,background:s.bg||'transparent',
    border:`1px solid ${s.cor}33`,padding:small?'2px 6px':'2px 9px',borderRadius:99,whiteSpace:'nowrap'}}>{s.label}</span>
}
...
function Avatar({nome,sz=26}){
  ...
  return <div style={{width:sz,height:sz,borderRadius:'50%',background:`${cor}22`,border:`1px solid ${cor}55`,
    display:'flex',alignItems:'center',justifyContent:'center',fontSize:sz*.38,fontWeight:800,color:cor,flexShrink:0}}>{ini}</div>
}
...
// Mini-selo da transportadora (sigla colorida por carrier)
const _TRANSP_CFG={'Loggi':{sig:'LG',cor:'#06b6d4'},'Jadlog':{sig:'JD',cor:'#ef4444'},'Correios':{sig:'CO',cor:'#f59e0b'},'J&T':{sig:'JT',cor:'#ec4899'},'Shopee Xpress':{sig:'SX',cor:'#f97316'},'Melhor Envio':{sig:'ME',cor:'#22c55e'}}
function TranspDot({nome,sz=16}){
  if(!nome)return null
  const k=Object.keys(_TRANSP_CFG).find(k=>String(nome).toLowerCase().includes(k.toLowerCase()))
  const c=_TRANSP_CFG[k]||{sig:String(nome).slice(0,2).toUpperCase(),cor:'#a78bfa'}
  return <span title={nome} style={{width:sz,height:sz,borderRadius:5,background:`${c.cor}22`,border:`1px solid ${c.cor}55`,
    display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:sz*.45,fontWeight:800,color:c.cor,flexShrink:0}}>{c.sig}</span>
}
...
// ─── PRODUCT THUMB ──────────────────────────────────────────────────────────
function ProductThumb({item,size=28,api}) {
  ...
  useEffect(()=>{
    ...
    fetch(`${api}/api/dashboard/produto-imagem?q=${encodeURIComponent(q)}`)
      .then(r=>r.json()).then(d=>{ if(d.url) setUrl(d.url) }).catch(()=>{})
  },[])
  if(url&&!err)return <img src={url} alt={name} onError={()=>setErr(true)}
...
function AnalyticsView({pedidos, api}) {
  ...
  // Enriquece top produtos: busca itens dos pedidos de hoje (máx 8 detalhes) de forma lazy
  useEffect(()=>{
    if(!pedidos.length) return
    const hoje=new Date().toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit',year:'2-digit'})
    const semItens=pedidos.filter(p=>fmtD(p.data)===hoje&&!(p.itens?.length)).slice(0,8)
    if(!semItens.length) return
    // Fire-and-forget: cada /pedido-completo traz itens; o componente re-renderiza quando os pedidos atualizam
    semItens.forEach(p=>{
      fetch(`${api}/api/dashboard/pedido-completo/${p.numero}`)
        .then(r=>r.ok?r.json():null)
        .then(d=>{ if(d?.itens?.length) Object.assign(p, { itens: d.itens }) })
        .catch(()=>{})
    })
  },[pedidos,api])
...
function KanbanCard({p, sit, onSel, api}) {
  ...
  return (
    <div onClick={()=>onSel(p)}
      ...
      <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:itens.length?6:8}}>
        <Avatar nome={p.contato} sz={17}/>
        <div style={{fontSize:10,color:'rgba(255,255,255,.65)',flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{p.contato||'—'}</div>
        {p.transportadora&&<TranspDot nome={p.transportadora} sz={14}/>}
      </div>
      {itens.length>0&&(
        <div style={{display:'flex',gap:3,marginBottom:7}}>
          {itens.slice(0,3).map((it,i)=><ProductThumb key={i} item={it} size={26} api={api}/>)}
  ...
  return <div style={{display:'grid',gridTemplateColumns:`repeat(${SIT_KANBAN_IDS.length},1fr)`,gap:10,alignItems:'start'}}>
    {SIT_KANBAN_IDS.map(sid=>{
      ...
      return <div key={sid} style={{display:'flex',flexDirection:'column',gap:6}}>
        ...
        {col.items.slice(0,14).map(p=><KanbanCard key={p.numero} p={p} sit={col.sit} onSel={onSel} api={api}/>)}
                  ...
                  {freteStats.length>0?(
                    freteStats.map(t=>{
                      ...
                      <div key={t.nome} style={{marginBottom:10,padding:atual?'6px 8px':0,borderRadius:atual?8:0,
                        ...
                        <div style={{fontSize:9,color:'var(--label-4)',marginTop:2}}>{t.pedidos} envios · {fmt(t.totalFrete)} total{t.tempoMedio!=null?` · ${t.tempoMedio}d envio`:''}</div>
                      </div>
                    )})
                  ):(
          ...
          : <>
            ...
            {pg.length===0
              ...
              : pg.map(p=>{
                ...
                const _transpRas=p.transportadora||null
                const cell=(colId)=>{
                  switch(colId){
