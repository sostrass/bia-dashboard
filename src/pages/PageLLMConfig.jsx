...
// ── Mensagens bypass ──────────────────────────────────────────────────────────
const MSGS = [
  { cat:'Checkout',  campo:'msgCpf',              titulo:'Solicitar CPF/CNPJ',           pad:'Me passa seu *CPF ou CNPJ* pra eu te encontrar aqui 😊', vars:null },
  { cat:'Endereço',  campo:'msgAlterarEnd',        titulo:'Solicitar novo CEP',            pad:'Me passa o *novo CEP* de entrega que busco o endereço pra você 😊', vars:null },
  { cat:'Endereço',  campo:'msgCepInvalido',       titulo:'CEP formato inválido',          pad:'Me passa o CEP com os 8 números certinhos, tá? (ex: 13482323)', vars:null },
  { cat:'Endereço',  campo:'msgCepNaoEncontrado',  titulo:'CEP não encontrado',            pad:'Não achei o CEP *{cep}* 🤔 Confere o número e me manda de novo?', vars:['{cep}'] },
  { cat:'Endereço',  campo:'msgErroCep',           titulo:'Erro ao consultar CEP',         pad:'Deu um erro aqui no CEP 😅 Tenta me passar de novo?', vars:null },
  { cat:'Carrinho',  campo:'msgQtdProduto',        titulo:'Quantidade — produto novo',     pad:'Quantas unidades de *{produto}* você quer? 😊', vars:['{produto}'] },
  { cat:'Carrinho',  campo:'msgQtdAlterar',        titulo:'Quantidade — alterar item',     pad:'Quantas unidades de *{produto}* você quer? (hoje tá {qtd_atual}x)', vars:['{produto}','{qtd_atual}'] },
  { cat:'Carrinho',  campo:'msgRemoverItem',       titulo:'Cabeçalho: remover item',       pad:'Qual item você quer tirar do carrinho?', vars:null },
  { cat:'Carrinho',  campo:'msgAlterarQtd',        titulo:'Cabeçalho: alterar quantidade', pad:'Qual item você quer alterar a quantidade?', vars:null },
  { cat:'Carrinho',  campo:'msgCarrinhoVazio',     titulo:'Carrinho esvaziado',            pad:'Carrinho zerado! 🗑️ O que mais você tá precisando?', vars:null },
  { cat:'Carrinho',  campo:'msgAdicionarItem',     titulo:'"Adicionar item" — pede produto',pad:'O que mais você tá buscando? 😊', vars:null },
  { cat:'Carrinho',  campo:'msgNovaBusca',         titulo:'Nova busca após limpar',        pad:'Me conta o que você tá procurando 😊', vars:null },
  { cat:'Fotos',     campo:'msgTodasFotos',        titulo:'Todas as fotos enviadas',       pad:'Essas são todas as {n} fotos desse produto! 📸', vars:['{n}'] },
  { cat:'Nota Fiscal', campo:'msgNotaFiscalPendente',  titulo:'NF não emitida — botão Emitir',   pad:'📄 *Nota Fiscal — Pedido #{numero_pedido}*\n\nOi, {nome_cliente}! A nota deste pedido ainda não foi emitida.\n\n📦 Pagamento confirmado ✅\n\nPosso emitir agora pra você!', vars:['{nome_cliente}','{numero_pedido}'] },
  { cat:'Nota Fiscal', campo:'msgNotaFiscalEmitida',   titulo:'NF autorizada — envia link',       pad:'📄 *Nota Fiscal — Pedido #{numero_pedido}*\n\nOi, {nome_cliente}! Aqui está sua nota 😊\n\n🔗 *Baixar NF-e:*\n{link_nfe}\n\n📋 NF-e nº {numero_nfe}', vars:['{nome_cliente}','{numero_pedido}','{numero_nfe}','{link_nfe}'] },
  { cat:'Nota Fiscal', campo:'msgNotaFiscalAguardando', titulo:'NF enviada — aguardando SEFAZ',   pad:'⏳ Emitindo sua nota fiscal... Já enviei pra SEFAZ e tô aguardando a autorização.\nAssim que sair, eu te mando o link aqui! 📄', vars:null },
  { cat:'Nota Fiscal', campo:'msgNotaFiscalProcessando', titulo:'NF em processamento (>30s)',     pad:'📄 Nota fiscal enviada pra SEFAZ! A autorização pode demorar alguns minutos.\nAssim que sair envio o link aqui 😊', vars:null },
  { cat:'Nota Fiscal', campo:'msgNotaFiscalSucesso',   titulo:'NF autorizada após emissão',       pad:'✅ *Nota emitida!* A SEFAZ autorizou 🎉\n\n🔗 *Baixar NF-e:*\n{link_nfe}\n\n📋 NF-e nº {numero_nfe}', vars:['{link_nfe}','{numero_nfe}'] },
  { cat:'Nota Fiscal', campo:'msgNotaFiscalFalha',     titulo:'NF com erro — abre ocorrência',    pad:'Poxa, não consegui emitir a nota automaticamente 😕\nNossa equipe foi avisada e entra em contato em breve. Protocolo: #{ticket_id}', vars:['{ticket_id}'] },
  { cat:'Histórico',   campo:'msgHistoricoPedidos',   titulo:'Histórico de pedidos — destaque do mais recente', pad:'Achei seus pedidos! 📦\n\n*Mais recente:* #{ultimo_pedido}\n📅 {data_ultimo} — {valor_ultimo}\n\nTem mais {outros_pedidos} no histórico. Qual você quer ver?', vars:['{nome_cliente}','{ultimo_pedido}','{data_ultimo}','{valor_ultimo}','{total_pedidos}','{outros_pedidos}'] },
  { cat:'Avaliação',   campo:'msgAvaliacaoEstrelas',  titulo:'Pergunta a nota (estrelas)',       pad:'Como foi a experiência com o pedido *#{numero_pedido}*? 😊', vars:['{numero_pedido}'] },
  { cat:'Avaliação',   campo:'msgAvaliacaoComentario',titulo:'Pergunta se quer comentar',        pad:'Eba, obrigada pela nota! 😊 Quer deixar um comentário rapidinho sobre como foi?', vars:null },
  { cat:'Avaliação',   campo:'msgAvaliacaoObrigada',  titulo:'Confirmação positiva (4-5★)',      pad:'Que alegria, {nome_cliente}! 🥰 Fico muito feliz em saber disso!', vars:['{nome_cliente}','{estrelas}'] },
  { cat:'Avaliação',   campo:'msgAvaliacaoNeutra',    titulo:'Confirmação neutra (3★)',           pad:'Obrigada, {nome_cliente}! 😊 A gente tá sempre buscando melhorar!', vars:['{nome_cliente}','{estrelas}'] },
  { cat:'Avaliação',   campo:'msgAvaliacaoNegativa',  titulo:'Confirmação negativa (1-2★) + ocorrência', pad:'Que pena, {nome_cliente} 😔 Sentimos muito. Nossa equipe vai entrar em contato pra resolver!', vars:['{nome_cliente}','{estrelas}'] },
  // ── Cenários de consulta de pedido ────────────────────────────────────────
  { cat:'Pedidos', campo:'msgMultiplosPedidos',      titulo:'Múltiplos pedidos — destaca o mais recente',  pad:'Achei *{total} pedidos* no seu cadastro! 📦\n\n*Mais recente:* #{ultimo_pedido}\n📅 {data_ultimo} — {valor_ultimo}\n\nQual você quer ver?\n[BOTOES: Nota fiscal #{ultimo_pedido} | Rastreio #{ultimo_pedido} | Ver outros pedidos]', vars:['{total}','{ultimo_pedido}','{data_ultimo}','{valor_ultimo}'] },
  ...
  { cat:'Pedidos', campo:'msgOutrosPedidos',          titulo:'Template lista de outros pedidos',           pad:'Olha seus últimos pedidos:\n1️⃣ #{num1} — {data1} — {valor1}\nQual você quer? Me manda o número 😊\n[BOTOES: Nota fiscal | Rastreio]', vars:['{num1}','{data1}','{valor1}'] },
]
