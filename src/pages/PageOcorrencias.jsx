...
// Espelho do mapa de Gatilhos — label + ícone + cor padronizados
const GATILHO_MAP = {
  ...
  pagamento_pendente: { label:'Pagamento Pendente',  icon:Clock,        cor:'#f59e0b' },
  pix_pendente:       { label:'PIX Pendente',        icon:CreditCard,   cor:'#06b6d4' },
  em_separacao:       { label:'Em Separação',        icon:Layers,       cor:'#8b5cf6' },
  produto_embalado:   { label:'Produto Embalado',    icon:Package,      cor:'#06b6d4' },
  nfe_pendente:       { label:'NF-e Pendente',       icon:FileTextIc,   cor:'#f59e0b' },
  nfe_emitida:        { label:'NF-e Emitida',        icon:FileTextIc,   cor:'#06b6d4' },
  nfe_rejeitada:      { label:'NF-e Rejeitada',      icon:AlertTriangle,cor:'#ef4444' },
  pedido_enviado:     { label:'Pedido Enviado',      icon:Truck,        cor:'#a78bfa' },
  pedido_coletado:    { label:'Pedido Coletado',     icon:Package,      cor:'#06b6d4' },
  rastreio_em_transito:{label:'Em Trânsito',         icon:Radio,        cor:'#4a9fff' },
  saiu_entrega:       { label:'Saiu para Entrega',   icon:Truck,        cor:'#f59e0b' },
  tentativa_entrega:  { label:'Tentativa de Entrega',icon:AlertTriangle,cor:'#f59e0b' },
  aguardando_retirada:{ label:'Aguardando Retirada', icon:Clock,        cor:'#0ea5e9' },
  endereco_incorreto: { label:'Endereço Incorreto',  icon:AlertTriangle,cor:'#ef4444' },
  nao_entrou_unidade: { label:'Não Entrou na Unidade',icon:AlertTriangle,cor:'#dc2626' },
  pedido_entregue:    { label:'Pedido Entregue',     icon:CheckCircle,  cor:'#22c55e' },
  nao_entregue:       { label:'Não Entregue',        icon:AlertTriangle,cor:'#ef4444' },
  pacote_devolvido:   { label:'Pacote Devolvido',    icon:RefreshCcw,   cor:'#f87171' },
  cancelamento:       { label:'Pedido Cancelado',    icon:XCircle,      cor:'#6b7280' },
  devolucao:          { label:'Devolução',           icon:RefreshCcw,   cor:'#f87171' },
  estorno_realizado:  { label:'Estorno Realizado',   icon:RefreshCcw,   cor:'#f97316' },
  avaliar_pedido:     { label:'Avaliação Pós-venda', icon:Star,         cor:'#f87171' },
  boas_vindas:        { label:'Boas-vindas',         icon:Star,         cor:'#e879f9' },
  avise_me:           { label:'Produto Disponível',  icon:Bell,         cor:'#fb923c' },
}
const gatilhoInfo = id => GATILHO_MAP[id] || { label:(id||'').replace(/_/g,' '), icon:Activity, cor:'#6b7294' }
  ...
  async function enviarWhats() {
    ...
    try {
      const r = await fetch(`${BASE}/api/ocorrencias/${oc.id}`, {
        ...
        body: JSON.stringify({ respostaCliente: resp.trim(), por: getAgente() }) })
      const d = await r.json()
      if (!r.ok) throw new Error(d.erro || 'Falha ao enviar')
      if (d.resposta_enviada === false) {
        setErro(d.resposta_erro ? `Não enviou: ${d.resposta_erro}` : 'A resposta foi salva, mas o WhatsApp não confirmou o envio (verifique o token).')
      } else { setResp('') }
              ...
              <Sec icon={Activity} color={T.purple} title="Disparos">
                {loading ? <Skel/> : (
                  <div style={{display:'flex', flexDirection:'column', gap:6, maxHeight:170, overflowY:'auto'}}>
                    {(ctx?.disparos||[]).length===0 && <div style={{fontSize:11.5, color:T.ink3}}>Nenhum disparo.</div>}
                    {(ctx?.disparos||[]).slice(0,10).map((d,i)=>{ const gi = gatilhoInfo(d.gatilho); const Ic = gi.icon; return (
                      <div key={i} style={{display:'flex', alignItems:'center', gap:8}}>
                        <div style={{width:22, height:22, borderRadius:7, flexShrink:0, background:`${gi.cor}18`, border:`1px solid ${gi.cor}40`, display:'flex', alignItems:'center', justifyContent:'center'}}>
                          <Ic size={12} color={gi.cor}/>
                        </div>
                        <span style={{flex:1, fontSize:11.5, fontWeight:600, color:T.ink2, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{gi.label}</span>
                        <span style={{width:6, height:6, borderRadius:99, flexShrink:0, background: d.status==='enviado'?T.green:T.red}} title={d.status}/>
                        <span style={{fontSize:10.5, color:T.ink4, flexShrink:0}}>{fmtRel(d.criado_em)}</span>
                      </div>
                    )})}
                  </div>
                )}
...
export default function PageOcorrencias() {
  ...
  const [modoMerge, setModoMerge] = useState(false)
        ...
        if (d.novos?.length) {
          setNovos(prev => {
            ...
            if (add.length) {
              ...
              if (temUrgente && typeof Notification!=='undefined' && Notification.permission==='granted')
                new window.Notification('Ocorrências — atenção', { body: add[0].nome_cliente ? `Novo ticket: ${add[0].nome_cliente}` : 'Novo ticket urgente' })
            }
  ...
  async function fundirSelecionados() {
    if (selecionados.length < 2) return
    const [alvo, ...origens] = selecionados   // primeiro selecionado = alvo
    const alvoTk = (dados.ocorrencias||[]).find(o=>o.id===alvo)
    if (!confirm(`Fundir ${origens.length} ticket(s) em ${alvoTk?.ticket_id}? Os demais serão encerrados e a timeline migrada.`)) return
    for (const oid of origens) {
      await fetch(`${BASE}/api/ocorrencias/${oid}/merge`, {
        method:'POST', headers:{'Content-Type':'application/json'},
    ...
    <div style={{minHeight:'100vh', height:'100vh', overflowY:'auto', background:T.bg0, padding:'22px 26px', fontFamily:'Inter, system-ui, sans-serif', fontSize:13}}>
      ...
      {modoMerge && selecionados.length > 0 && (
        <div style={{position:'sticky', top:0, zIndex:30, display:'flex', alignItems:'center', gap:12, padding:'11px 16px', marginBottom:12, background:T.purpleDim, border:`1px solid ${T.purpleBor}`, borderRadius:13, boxShadow:`0 8px 30px -10px ${T.purple}66`}}>
          <GitMerge size={16} color={T.purple}/>
          <span style={{fontSize:12.5, color:T.ink1, fontWeight:700}}>{selecionados.length} selecionado{selecionados.length>1?'s':''}.</span>
          <span style={{fontSize:11.5, color:T.ink3}}>O 1º vira o ticket principal; os demais são fundidos nele.</span>
          <button onClick={fundirSelecionados} disabled={selecionados.length<2}
            style={{marginLeft:'auto', display:'flex', alignItems:'center', gap:6, background: selecionados.length>=2?T.purple:T.bg3, border:'none', color: selecionados.length>=2?'#fff':T.ink4, borderRadius:9, padding:'7px 16px', cursor: selecionados.length>=2?'pointer':'default', fontWeight:900, fontSize:12.5}}>
            <GitMerge size={13}/>Fundir {selecionados.length} tickets
          </button>
          <button onClick={()=>setSelecionados([])} style={{background:'none', border:'none', color:T.ink3, cursor:'pointer', fontSize:12}}>limpar</button>
        </div>
      )}
      ...
      <div style={{display:'flex', flexDirection:'column', gap:9, maxHeight: sel ? 'calc(100vh - 150px)' : 'none', overflowY: sel ? 'auto' : 'visible', paddingRight: sel ? 4 : 0}}>
        ...
        {!loading && lista.map(o => {
          ...
          return (
            <div key={o.id} onClick={()=> modoMerge ? toggleSel(o.id) : setSel(o)}
              style={{display:'flex', alignItems:'center', gap: sel?9:13, padding: sel?'9px 11px':'12px 15px', cursor:'pointer',
                background: selecionados.includes(o.id) ? T.purpleDim : sel?.id===o.id ? T.bg3 : T.bg1,
                border:`1px solid ${selecionados.includes(o.id) ? T.purpleBor : sel?.id===o.id ? T.blueBor : urgente?T.redBor:T.sep2}`, borderRadius:14,
                boxShadow: urgente && !modoMerge ? `0 0 26px -12px ${T.red}88` : 'none',
                transition:'all .15s'}}
              onMouseEnter={e=>{ if(!selecionados.includes(o.id)){e.currentTarget.style.background=T.bg2} }}
              onMouseLeave={e=>{ if(!selecionados.includes(o.id)){e.currentTarget.style.background= sel?.id===o.id?T.bg3:T.bg1} }}>
              {modoMerge && (
