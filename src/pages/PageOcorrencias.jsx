        ...
        {!loading && lista.map(o => {
          ...
          return (
            <div key={o.id} onClick={()=>setSel(o)}
              ...
              <Avatar nome={o.nome_cliente} size={sel?34:40}/>
              <div style={{flex:1, minWidth:0}}>
                <div style={{display:'flex', alignItems:'center', gap:6, flexWrap:'wrap'}}>
                  <span style={{fontSize: sel?12.5:13.5, fontWeight:800, color:T.ink1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth: sel?140:'none'}}>{o.nome_cliente || fmtTel(o.telefone) || 'Cliente'}</span>
                  <span style={{fontSize:10.5, color:T.ink4, fontWeight:700}}>{o.ticket_id}</span>
                  {!sel && <Bdg color={tp.color} dim={tp.dim} bor={tp.bor} icon={tp.icon}>{tp.label}</Bdg>}
                  {!sel && <CanalChip canal={o.canal}/>}
                  {!sel && o.numero_pedido && <span style={{fontSize:11, color:T.cyan}}>#{o.numero_pedido}</span>}
                </div>
                <div style={{fontSize: sel?11.5:12, color:T.ink3, marginTop:2, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>
                  {sel && <span style={{color:tp.color, fontWeight:700}}>{tp.label} · </span>}{o.titulo || o.descricao}
                </div>
              </div>
              <div style={{display:'flex', alignItems:'center', gap: sel?6:10, flexShrink:0}}>
                {o.followup_vencido && <Bdg color={T.red} dim={T.redDim} bor={T.redBor} icon={AlarmClock}>{sel?'':'follow-up'}</Bdg>}
                {!sel && o.escalonado && <Bdg color={T.orange} dim={T.orangeDim} bor={T.orangeBor} icon={TrendingUp}>SLA⬆</Bdg>}
                {urgente && <Zap size={13} color={T.red}/>}
                {!sel && <span style={{fontSize:11, color:pr.color, fontWeight:800}}>{pr.label}</span>}
                <Bdg color={sd.color} dim={sd.dim} bor={sd.bor} icon={sd.icon} size="sm">{sel?'':sd.label}</Bdg>
                <span style={{fontSize:10.5, color:T.ink4, minWidth: sel?28:38, textAlign:'right'}}>{fmtRel(o.atualizado_em || o.criado_em)}</span>
                {!sel && <ChevronRight size={15} color={T.ink4}/>}
              </div>
            </div>
          )
        })}
